...
import { formatEuro, parseEuroToCents, parseNonNegativeEuroToCents, parseSignedEuroToCents } from './money'
...
describe('money boundaries', () => {
  ...
  it('accepts zero for amounts that may be cleared, unlike the strictly positive parser', () => {
    expect(parseNonNegativeEuroToCents('0')).toBe(0)
    expect(parseNonNegativeEuroToCents('12,34')).toBe(1234)
    expect(() => parseNonNegativeEuroToCents('texto')).toThrow('importe válido')
  })
