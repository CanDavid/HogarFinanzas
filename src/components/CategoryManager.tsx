...
export function CategoryManager({ categories, onCreate, onUpdate, onArchive }: {
  categories: Category[]; onCreate(input: CategoryInput): Promise<void>; onUpdate(id: string, input: CategoryInput): Promise<void>; onArchive(id: string): Promise<void>
}) {
  ...
  return <section className="management"><div className="section-title"><h2>Categorías</h2><span>{categories.filter((item) => !item.archivedAt).length}</span></div>
    ...
    <ul>{categories.map((category) => <li key={category.id} className={category.archivedAt ? 'archived' : ''}>
      ...
      {!category.archivedAt && <button className="delete" onClick={() => void onArchive(category.id)}>Archivar</button>}
    </li>)}</ul></section>
}
