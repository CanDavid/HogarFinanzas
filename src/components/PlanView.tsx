...
import { formatEuro, parseEuroToCents, parseNonNegativeEuroToCents } from '../domain/money'
...
function BudgetSection({ month, progress, categories, locked, onSave }: { month: string; progress: ReturnType<typeof buildMonthlyPlan>['budgets']; categories: Category[]; locked: boolean; onSave(categoryId: string, amount: number): Promise<void> }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setSaving(true); setError(''); try { await onSave(categoryId, parseNonNegativeEuroToCents(amount)); setAmount('') } catch (cause) { setError(cause instanceof Error ? cause.message : 'No se pudo guardar el presupuesto.') } finally { setSaving(false) } }
...
function DistributionSection({ month, summary, current, locked, onSave }: { month: string; summary: ReturnType<typeof buildMonthlyPlan>['summary']; current?: MonthlyPlan; locked: boolean; onSave(savings: number, investment: number): Promise<void> }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setSaving(true); setError(''); try { await onSave(parseNonNegativeEuroToCents(savings), parseNonNegativeEuroToCents(investment)) } catch (cause) { setError(cause instanceof Error ? cause.message : 'No se pudo guardar la distribución.') } finally { setSaving(false) } }
