...
export function AccountManager({ accounts, balances, onCreate, onUpdate, onArchive, onRestore, onAdjustBalance, onSelectAccount }: {
  ...
  onAdjustBalance?(id: string): void; onSelectAccount?(id: string): void
}) {
  ...
  return <section className="management"><div className="section-title"><h2>Cuentas</h2><span>{accounts.filter((item) => !item.archivedAt).length}</span></div>
    {!onSelectAccount && <AccountForm key={editing?.id ?? 'new'} account={editing} onSave={async (input) => { if (editing) await onUpdate(editing.id, input); else await onCreate(input); setEditing(null) }} onCancel={editing ? () => setEditing(null) : undefined} />}
    <ul>{accounts.map((account) => <li key={account.id} className={account.archivedAt ? 'archived' : ''}>
      <button className="management-main" onClick={() => (onSelectAccount ? onSelectAccount(account.id) : setEditing(account))}><strong>{account.name}</strong><small>{accountTypeLabel(account.type)} · {formatEuro(balances.get(account.id) ?? account.initialBalanceCents)}</small></button>
      {!onSelectAccount && <div className="management-actions">{!account.archivedAt && onAdjustBalance && <button className="text-action" onClick={() => onAdjustBalance(account.id)}>Ajustar saldo</button>}
